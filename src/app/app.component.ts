...
polygonSeries.events.once("inited", function(){
  ...
  polygonSeries.mapPolygons.each(function(polygon) {
    ...
    let cflagData = {
      ...
      "title": (polygon.dataItem.dataContext as any).name,
      "flag": (polygon.dataItem.dataContext as any).flag
    }
...
polygonTemplate.events.on("hit", function(ev) {
  if(qDisplay.disabled) {
    ...
    if ((data as any).background) {
      ...
      let modal = chart.openModal("<img src=" + (data as any).flag + " style=\"width:128px;height:64px;\">" + "<strong><p style=\"font-size:30px\">" + (data as any).name + " (" + (data as any).id + ")" + "</p></strong>" + (data as any).background + "<a href=\"https://en.wikipedia.org/wiki/" + (data as any).name.replace(' ','_') + "\"target=\"_blank\">More info</a>", (data as any).name);
...
function displayQuestion() {
  ...
  polygonSeries.mapPolygons.each(function(mapPolygon) {
    if(mapPolygon.dataItem.dataContext && (mapPolygon.dataItem.dataContext as any).id != worldData[rc1].id && mapPolygon.dataItem.dataContext && (mapPolygon.dataItem.dataContext as any).id != worldData[rc2].id && mapPolygon.dataItem.dataContext && (mapPolygon.dataItem.dataContext as any).id != worldData[rc3].id && mapPolygon.dataItem.dataContext && (mapPolygon.dataItem.dataContext as any).id != worldData[rc4].id)
      mapPolygon.setState("nullified");
    else
      mapPolygon.setState("active")
    mapPolygon.defaultState.properties.fill = undefined
